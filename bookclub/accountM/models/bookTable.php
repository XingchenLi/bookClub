...
class BookTable {
    ...
    protected $_nav_bar_item = array();

    protected $_menu_state_field;
    protected $_menu_style_field;

    public function __construct($rows = array(), $menu_items = array(),
    $_nav_bar_item = array(), $menu_state_field = null, $menu_style_field = null) {
        $this->_columns = $columns;
        $this->_rows = $rows;
        $this->_menu_items = $menu_items;
        $this->_nav_bar_item = $_nav_bar_item;
    ...
    public function render(){
        ...
        // Add navbar, if specified
        if (!empty($this->_nav_bar_item)){
            $result .= "<th>{$this->_menu_label} <i class='fa fa-sort'></i></th>";
        }

        $result .= "</tr><p0/thead><tbody>";
